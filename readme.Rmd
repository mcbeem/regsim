...
Let us imagine that we can measure variable $X$ with a reliability of 0.8. According to classical test theory, $X$ is composed of a true score component $X_{true}$, a measurement error component $X_{error}$. The observed values of this variable, which are contaminated with measurement error, is $X_obs$. $X_{true}$ and $X_{error}$ are latent variables that cannot be observed.

The situation can be represented by the following path diagram.

```{r, fig.width=4.5, fig.height=2.5, echo=F}
dag <- dagitty("dag{Y <- X_true -> X_obs <- X_error }")
coordinates(dag) <- list(x=c(Y=1, X_true=0, X_error=0, X_obs=.5), 
                         y=c(Y=.5, X_true=0, X_error=1, X_obs=.5))
exposures(dag) <- "X_true"
outcomes(dag) <- "Y"
plot(dag)
```

Note that there is no path from $X_obs$ to $Y$. $Y$ is caused by the true value, $X_{true}$, and not by the observed $X_{obs}$. The path coefficients $X_{true} \rightarrow X_{obs}$ and $X_{error} \rightarrow X_{obs}$ can be manipulated to control the reliability coefficient of $X_{obs}$. 

Let us imagine the exogenous variables, $X_{true}$ and $X_{error}$, exist on a standardized metric and thus both have variances of one. According to classical test theory, the observed score equals the true score plus error.

The path coefficient for $X_{error} \rightarrow X_{obs}$ is also dictated by the desired reliability. It should be set to one minus the square root of the reliability.
...
Next, we run the simulation, being sure to set the ```standardized=``` argument to ```TRUE```. Also note that the target value I have set is equal to the effect of $X_true \rightarrow Y$, not the zero effect of $X_{obs} \rightarrow Y$. 
...
The results indicate that the estimated effect of $X \rightarrow Y$ is biased. The reason is that the unobservable true score $X_{true}$ is a confounder. This is seen in the ```$adjustment.sets``` component of the output. Technically we might note that our analysis model isn't even correct, because we have replaced the latent $X_{true}$ with its proxy variable, $X_{obs}$. But it's the best that can be done given the variables we have. Thus, it is clear that measurement error bias is no different than confounding bias. Both forms are caused by omitted variables.


## Measurement error in the response variable

```{r, fig.width=4.5, fig.height=2.5, echo=F}
dag <- dagitty("dag{X -> Y_true -> Y_obs <- Y_error }")
coordinates(dag) <- list(x=c(X=0, Y_true=1, Y_error=1, Y_obs=.5), 
                         y=c(X=.5, Y_true=0, Y_error=1, Y_obs=.5))
exposures(dag) <- "X"
outcomes(dag) <- "Y_true"
plot(dag)
```
