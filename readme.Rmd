...
Let us imagine that we can measure variable $X$ with a reliability of 0.7. According to classical test theory, the observed score $X_{obs}$ is composed of a true score component $X_{true}$ plus measurement error component $X_{error}$. 
...
Next, we run the simulation. Also note that the target value I have set is equal to the effect of $X_true \rightarrow Y$, not the zero effect of $X_{obs} \rightarrow Y$. 
